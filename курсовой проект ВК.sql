...
-- Описание: 
--   База данных, на подобие социальной сети ВК. База хранит информацию о пользователях,друзьях пользователей, о медиа-контенте, который выкладывают пользователи
--   о том, что пользователям нравится ( лайки), переписка между пользователями, членство пользователей в различных группах.
...
--   6. Messages
--   7. Frendship
--   8. Frendship_statuses
--   9. Media
--   10. media_types
--   11. Likes
--   12. target_types
--   13. posts
--   14. TEMPORARY TABLE genders
--   15. TEMPORARY TABLE filetype 
-- Большинство таблиц сначала заполнялись дамми данными с сайта http://filldb.info, потом корректировались, устранялись ошибки, чтобы выглядеть реалистично.

-- Запросы:
--    Подсчитать средний возраст пользователей.
--    Подсчитать количество дней рождения, которые приходятся на каждый из дней недели.
--    Определить кто больше поставил лайков (всего) - мужчины или женщины?
--    Подсчитать количество лайков которые получили 10 самых молодых пользователей.
--    Найти 10 пользователей, которые проявляют наименьшую активность в использовании социальной сети.
--    Найти 10 пользователей, которые проявляют наибольшую активность в использовании социальной сети.

-- Представления:
--   Представление, которое выводит сообщения от пользователя
--   Представление, которое выводит имя и фамилию пользователей с сортировкой по фамилии

-- Хранимая функция:
--    hello(), которая будет возвращать приветствие
...
CREATE TABLE users (
id INT(10) UNSIGNED PRIMARY KEY COMMENT "ID пользователя",
...
CREATE TABLE profiles (
  user_id INT(10) UNSIGNED PRIMARY KEY COMMENT "Ссылка на пользователя", 
...
ALTER TABLE profiles
  ADD CONSTRAINT profiles2_user_id_fk 
...
-- исправляем ошибки дамми данных
UPDATE profiles SET updated_at = NOW() WHERE updated_at < created_at;  #  корректируем даты 

UPDATE profiles SET photo_id = (SELECT FLOOR(1+RAND()*100)); #делаем id на фото до 100

DROP TABLE IF EXISTS genders;
CREATE TEMPORARY TABLE genders (name CHAR(1)); # меняем пол на русские буквы
...
-- делаем статусы пользователей понятными, значения до 4 из таблицы user_status
UPDATE profiles SET status = NULL; #сначала обнулим все значения в этом столбе таблицы
ALTER TABLE profiles RENAME COLUMN status TO status_id; # переименуем
ALTER TABLE profiles MODIFY COLUMN status_id INT UNSIGNED; #дадим другой тип
UPDATE profiles SET status_id = 1+FLOOR(RAND()*4); # заполним
...
CREATE TABLE user_status (
  id INT(10) UNSIGNED PRIMARY KEY COMMENT "Ссылка на пользователя", 
...
-- заполняем таблицу
INSERT INTO user_status(id,name) VALUES ('1','single');
INSERT INTO user_status(id,name) VALUES ('2','married');
INSERT INTO user_status(id,name) VALUES ('3','too dificult');
INSERT INTO user_status(id,name) VALUES ('4','active search');
...
CREATE TABLE messages (
  id INT(10) UNSIGNED PRIMARY KEY COMMENT "Идентификатор строки", 
...
ADD CONSTRAINT message2_fromuser_id_fk
    FOREIGN KEY (from_user_id) REFERENCES users(id)
      ON DELETE RESTRICT,
ADD CONSTRAINT message2_touser_id_fk
...
-- удаляем старые данные и меняем данные на понятные
TRUNCATE friendship_statuses; 
INSERT INTO friendship_statuses(id,name) VALUES ('1','confirmed'),('2','reject'),('3','doubt');

-- меняем статусы  в таблице friendship на статусы из таблицы статусов
...
CREATE TABLE communities (
  id INT(10) UNSIGNED PRIMARY KEY COMMENT "Идентификатор сроки",
...
ALTER TABLE communities_users 
  ADD CONSTRAINT communities2_users_users_id_fk
    FOREIGN KEY (user_id) REFERENCES users(id)
      ON DELETE CASCADE ;
     
ALTER TABLE communities_users      
ADD CONSTRAINT communities2_users_community_id_fk
...
-- решили оставить только 20 групп, а не 100
DELETE FROM communities WHERE id > 20; # deleted lines from 20 to 100

-- обновили связанную таблицу
...
ALTER TABLE media  
ADD CONSTRAINT media2_user_id_fk
    FOREIGN KEY (user_id) REFERENCES users(id)
      ON DELETE RESTRICT;
...
-- Заполняем лайки рандомными данными
INSERT INTO likes 
  SELECT 
    id, 
    FLOOR(1 + (RAND() * 100)), 
...
CREATE TABLE target_types (
  ...
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP
)  COMMENT "Типы данных, которые нравятся пользователям";
-- заполняем таблицу
...
-- добавляем внешние ключи
ALTER TABLE likes   
ADD CONSTRAINT likes2_user_id_fk
    FOREIGN KEY (user_id) REFERENCES users(id)
      ON DELETE RESTRICT,
ADD CONSTRAINT target2_type_id_fk
...
-- добавляем внешние ключи
     ALTER TABLE posts   
ADD CONSTRAINT post2_user_id_fk
    FOREIGN KEY (user_id) REFERENCES users(id)
      ON DELETE RESTRICT,
ADD CONSTRAINT post2_media_id_fk
    FOREIGN KEY (media_id) REFERENCES media(id)
      ON DELETE SET DEFAULT ;
    ALTER TABLE posts   
ADD CONSTRAINT post2_community_id_fk
    FOREIGN KEY (community_id) REFERENCES communities(id)
      ON DELETE SET DEFAULT;  
     
     
-- добавляем внешние ключи в таблицу профайл
ALTER TABLE profiles
  ADD CONSTRAINT profiles2_status_id_fk
    FOREIGN KEY (status_id) REFERENCES user_status(id)
      ON DELETE SET NULL;     
     
-- добавляем внешние ключи в таблицу дружбы
ALTER TABLE friendship    
ADD CONSTRAINT friendship2_user_id_fk
    FOREIGN KEY (user_id) REFERENCES users(id)
      ON DELETE CASCADE,
ADD CONSTRAINT friendship2_friend_id_fk
    FOREIGN KEY (friend_id) REFERENCES users(id)
      ON DELETE CASCADE,
ADD CONSTRAINT friendship2_status_id_fk
    FOREIGN KEY (status_id) REFERENCES friendship_statuses(id)
      ON DELETE CASCADE;    

     -- добавляем внеший ключ в профайлы
ALTER TABLE profiles
  ADD CONSTRAINT profiles2_photo_id_fk
...
SELECT YEAR(NOW()), MONTH(birthday), DAY(birthday) from profiles p ;
SELECT DATE(CONCAT_WS('-',YEAR(NOW()), MONTH(birthday), DAY(birthday))) from profiles p ;
SELECT DATE_FORMAT(DATE(CONCAT_WS('-',YEAR(NOW()), MONTH(birthday), DAY(birthday))),'%W') from profiles p ;
SELECT DATE_FORMAT(DATE(CONCAT_WS('-',YEAR(NOW()), MONTH(birthday), DAY(birthday))),'%W') as day from profiles p ;
SELECT DATE_FORMAT(DATE(CONCAT_WS('-',YEAR(NOW()), MONTH(birthday), DAY(birthday))),'%W') as day from profiles p2 group by day;
SELECT DATE_FORMAT(DATE(CONCAT_WS('-',YEAR(NOW()), MONTH(birthday), DAY(birthday))),'%W') as day,COUNT(*) as total from profiles p2 group by day ORDER by total desc;
...
-- Создать представление, которое выводит сообщения от пользователя
CREATE OR REPLACE VIEW users_messages AS
SELECT
  u.last_name AS userss,
  m.body AS message 
FROM
  users AS u
JOIN 
...
ON
  u.id = m.from_user_id;
 SELECT * FROM  users_messages limit 5;



-- Создать представление, которое выводит имя и фамилию пользователей с сортировкой по фамилии
CREATE OR REPLACE VIEW usr AS SElECT first_name,last_name from users u ORDER BY last_name ;
SELECT * FROM usr;

 
 
 
-- Хранимая функция hello(), которая будет возвращать приветствие,
